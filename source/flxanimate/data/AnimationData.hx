...
class AnimationData
{
	...
	public static function fromFilterJson(filters:Filters = null):Null<FlxShader>
	{
		if (filters == null) return null;
		
		var bitmapFilter:Null<FlxShader> = null;
		...
		for (filter in Reflect.fields(filters))
		{
			switch (filter)
			{
				/*case "GF", "GlowFilter":
				{
					...
					trace('added glow filter, the array is now ' + bitmapFilter);
				}*/
				case "BLF", "BlurFilter":
				{
					var blur:BlurFilter = Reflect.field(filters, filter);
					trace('added blur filter');
					return new BlurShader(blur.BLX, blur.BLY/*, blur.Q*/);
				}
			}
		}

		if(bitmapFilter == null) trace('dang it its still null');
...
abstract Filters({})
{
	...
	{
		return Reflect.fields(this).toString();
	}
}
/**
