...
class FlxAnimate extends FlxSprite
{
	...
	function parseElement(instance:FlxElement, curFrame:Int, m:FlxMatrix, colorFilter:ColorTransform, mainSymbol:Bool = false)
	{
		...
		{	
			drawLimb(frames.getByName(instance.bitmap), matrix, colorEffect, instance.symbol?.bitmapShader ?? null);
			return;
		}
	...
	function drawLimb(limb:FlxFrame, _matrix:FlxMatrix, ?colorTransform:ColorTransform, ?bitmapShader:flixel.system.FlxAssets.FlxShader = null)
	{
		...
		for (camera in cameras)
		{
			...
			camera.drawPixels(limb, limb.parent.bitmap, rMatrix, colorTransform, blend, antialiasing, bitmapShader);
			#if FLX_DEBUG
