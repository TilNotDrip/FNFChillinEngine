...
class PlayState extends MusicBeatState
{
	...
	public var upcomingNotes:Array<SwagNote>;
	public var upcomingEvents:Array<SwagEvent>;
	...
	override public function create()
	{
		...
		seperateVocals = !Paths.location.exists('${SONG.metadata.song.formatToPath()}/Voices.${Constants.EXT_SOUND}', 'songs', MUSIC);

		FlxG.sound.cache(Paths.location.inst(SONG.metadata.song));

		if (seperateVocals)
		{
			FlxG.sound.cache(Paths.location.voices(SONG.metadata.song, SONG.metadata.player.split('-')[0].trim()));
			FlxG.sound.cache(Paths.location.voices(SONG.metadata.song, SONG.metadata.opponent.split('-')[0].trim()));
		}
		else
			FlxG.sound.cache(Paths.location.voices(SONG.metadata.song));
	...
	public function startCountdown()
	{
		...
		Module.callFunction('startCountdown');
	...
	{
		var spr:FlxSprite = new FlxSprite().loadGraphic(Paths.content.image(path));
	...
	public function startSong():Void
	{
		...
		if (!paused)
			FlxG.sound.playMusic(Paths.location.inst(SONG.metadata.song), 1, false);
	...
	public function generateSong():Void
	{
		if (seperateVocals)
		{
			for (char in [SONG.metadata.player, SONG.metadata.opponent])
			{
				...
				var musicData = new FlxSound().loadEmbedded(Paths.location.voices(SONG.metadata.song, newChar));
		...
		{
			var musicData = new FlxSound().loadEmbedded(Paths.location.voices(SONG.metadata.song));
		...
		for (songNotes in SONG.notes)
		{
			...
			var daNoteData:Int = getNoteId(songNotes);
			...
			var swagNote:Note = new Note(songNotes, (gottaHitNote) ? boyfriend.isPixel : dad.isPixel);
			swagNote.scrollFactor.set(0, 0);
			swagNote.strums = ((songNotes.strum == 'Player') ? playerStrums : opponentStrums);
	...
	function getNoteId(note:SwagNote):Int
	{
		if (Modifiers.hasModifierOn('random-charts'))
		{
	...
	override public function closeSubState()
	{
		if (paused)
		{
			Module.callFunction('resumeGame');
			...
			for (icon in [iconP1, iconP2])
				if (icon.daTween != null && !icon.daTween.finished)
					icon.daTween.active = true;
	...
	override public function update(elapsed:Float)
	{
		...
		if (ChillSettings.get('hudType') == 'Complex')
			songTxt.text = '['
				...
				+ '] ['
				+ FlxMath.roundDecimal((FlxG.sound.music.time / FlxG.sound.music.length) * 100, 2)
				+ '%] '
				+ SONG.metadata.song
				+ ' - '
				+ difficulty;
	...
	override public function draw()
	{
		...
		holdNotes.forEach(function(daNote:SustainNote)
		{
			...
			if (ChillSettings.get('downScroll'))
			{
				daNote.y = daNote.head.y - (Conductor.stepCrochet * (0.45 * FlxMath.roundDecimal(SONG.metadata.speed, 2)));
				daNote.flipY = true;
			}
			else
				daNote.y = daNote.head.y + (Conductor.stepCrochet * (0.45 * FlxMath.roundDecimal(SONG.metadata.speed, 2)));
		...
		notes.forEachAlive(function(daNote:Note)
		{
			...
			daNote.x = whatStrum.x + (Note.NOTE_WIDTH * daNote.data.direction);
	...
	private function pauseGame()
	{
		if (canPause && !isEnding)
		{
			...
			for (icon in [iconP1, iconP2])
				if (icon.daTween != null)
					icon.daTween.active = false;

			Module.callFunction('pauseGame');
	...
	public function finishSongStory()
	{
		...
		while (storyWeek.songs[curSongIndex] == null && storyWeek.songs[curSongIndex].difficulties.contains(difficulty))
		{
			trace('Updated index due to an issue!');
			curSongIndex++;
		}
	...
	{
		var noteDiff:Float = Math.abs(daNote.data.time - Conductor.songPosition);
		for (song in vocals)
			song.volume = 1;
		...
		if (daRating == 'sick' && ChillSettings.get('noteSplashes'))
		{
			...
			noteSplashPlayer.setColors(daNote.returnColors());
			playerSplashes.add(noteSplashPlayer);
		}
		...
		rating.loadGraphic(Paths.content.image(ratingPath));
	...
	private function displayCombo():Void
	{
		...
		var comboSpr:FlxSprite = new FlxSprite().loadGraphic(Paths.content.image(pixelShitPart1 + 'ui/combo'));
		...
		{
			var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.content.image(pixelShitPart1 + 'ui/num' + Std.int(i)));
	...
	var currentlyHeld:Array<Bool> = [false, false, false, false];
	var inputReleaseQueue:Array<Int> = [];

	function processInputQueue():Void
	{
		if (inputPressQueue.length + inputReleaseQueue.length == 0)
			return;
		...
		notes.forEachAlive(function(note:Note)
		{
			if (note.data.strum == 'Player' && note.mayHit && !note.wasHit)
				notesInRange.push(note);
		});

		notesInRange.sort(sortByShit); // THIS FIXES THE FUCKING INPUT!!!
		...
		for (note in notesInRange)
			notesByDirection[note.data.direction].push(note);
	...
	public var currentHoldNotes:Array<Note> = [];
	...
	public function handleHoldNotes()
	{
		...
		for (note in currentHoldNotes)
			byDirection[note.data.direction].push(note);
		...
		{
			for (strumHead in curHoldCovers.keys())
			{
				for (i => holdCover in curHoldCovers.get(strumHead))
				{
					if (holdCover != null && !holdCover.alive)
						curHoldCovers.get(strumHead)[i] = null;
					else
						continue;
				}
			}
		...
		for (i in 0...byDirection.length)
		{
			...
			while (byDirection[i].length > 0)
			{
				...
				if (daNote == null)
					daNote = byDirection[i][0];
				if (daNote == null)
					continue;

				var whatStrum:Strums = daNote.strums;
				var curHoldCover:NoteHoldCover = curHoldCovers.get(daNote.data.strum)[i];

				if (curHoldCover == null)
				{
					curHoldCover = holdCovers.recycle(NoteHoldCover);
					curHoldCover.setColors(daNote.returnColors());
					curHoldCover.setPosition(whatStrum.x + (Note.NOTE_WIDTH * i) - 20, whatStrum.y - 10);
					curHoldCover.playStart();
					holdCovers.add(curHoldCover);

					curHoldCovers.get(daNote.data.strum)[i] = curHoldCover;
				}

				if (daNote.sustain.y + daNote.sustain.offset.y * daNote.sustain.scale.y <= strumLineMid)
				{
					var swagRect:FlxRect = new FlxRect(0, 0, daNote.sustain.width / daNote.sustain.scale.x, daNote.sustain.height / daNote.sustain.scale.y);

					swagRect.y = (strumLineMid - daNote.sustain.y) / daNote.sustain.scale.y;
					swagRect.height -= swagRect.y;
					daNote.sustain.clipRect = swagRect;

					if (curHoldCover != null && daNote.animation.curAnim.name.endsWith("end") && swagRect.height <= 5)
					{
	...
	{
		Module.callFunction('noteMiss', [note]);
		missThing();
	...
	override public function stepHit()
	{
		...
		for (song in vocals)
		{
			if (!didIt
				...
				|| (Math.abs(song.time - (Conductor.songPosition - Conductor.offset)) > 20))
			{
				resyncVocals();
				didIt = true;
			}
	...
	function runFollowTween(?ease:String = 'lerp', ?time:Float = 0.26)
	{
		...
		if (ease != 'lerp')
			FlxTween.tween(camGAME.scroll, {x: followPos.x, y: followPos.y}, time, {
				ease: Reflect.field(FlxEase, ease) ?? FlxEase.linear,
				onUpdate: function(twn:FlxTween)
				{
					camGAME.scroll.x = (!isPixel) ? camGAME.scroll.x : (Math.round(camGAME.scroll.x / 6) * 6);
					camGAME.scroll.y = (!isPixel) ? camGAME.scroll.y : (Math.round(camGAME.scroll.y / 6) * 6);
				}
			});
		else
		{
			...
			{
				camGAME.scroll.x = (!isPixel) ? value : (Math.round(value / 6) * 6);
