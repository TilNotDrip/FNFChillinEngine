/**
 * This is a secret.
 */
class MyAsshole // :troll:
{
	public function new()
	{
		throw "This isnt finished yet!! expect it in the near future tho";
	}

	/**
	 * please never fucking use this
	 * lol (yea 𝓕𝓻𝓮𝓪𝓴𝔂GPT)
	 * @param speed How fast you wanna *go*?
	 * @return feel good, or not.
	 */
	public function jerkingOff(speed:Float):Void
	{
		// i made this with crusher for no fucking reason lol 🤑🤑🤑🤑🤑🤑🤑🤑🤑🤑(❁´◡`❁)☆*: .｡. o(≧▽≦)o .｡.:*☆(*/ω＼*)(●'◡'●)╰(*°▽°*)╯(^///^)
		// stop jerking off crusher
		// I DONT WANT TO
		new FlxTimer().start(64 / speed, function(tmr:FlxTimer)
		{
			cocksexbuttasscock('kum'); // go wild 😏😼
		});
	}

	/**
	 * im boutta KUHJJOnjfrjhkldsahjkfhgjkldsahjkngfdjkl;gjkl;fdaslkhjgfdsjkl,'gmf'j klsd;gh'l;jkmfsdlmkj';gfk;l'sdmk'glm;fsd'kml;b'kl;vcs'k;mlhgsd'kl;mgh'klf;md
	 * s'kl;m bvfd'kml;ng'bdfklm;h'mkl;tf'km;lh'gdf
	 * @param what you are about to do
	 * @return Application is not responding.
	 */
	function cocksexbuttasscock(what:String):Void
	{
		var holdon:String = '';

		for (i in 0...FlxMath.MAX_VALUE_INT)
		{
			holdon += what.charAt(what.length - 1).toUpperCase();
		}

		trace('im bouta ' + what.toUpperCase() + holdon);
	}
}
